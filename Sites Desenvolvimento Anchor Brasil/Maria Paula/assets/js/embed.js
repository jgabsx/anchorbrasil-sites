!function(t, e) {
    ...
    m.prototype.open = function() {
        this.loadIframe(),
        this.paddingBottom ? this.content.style.paddingBottom = this.paddingBottom : this.content.removeAttribute("style"),
        this.el[s].add(c),
        this.el[s].add(r),
...
EMBEDSOCIALHASHTAG = {
    getEmbedData: function(albumRef, albumDiv) {
        ...
        if (widgetTitleAttributeValue) {
            if (queryParamString) {
                queryParamString = queryParamString + "&overridenTitle=" + widgetTitleAttributeValue
            } else {
                queryParamString = "?overridenTitle=" + widgetTitleAttributeValue
            }
        }
        if (typeof window !== "undefined" && typeof window.encodeURIComponent === "function" && window.encodeURIComponent(EMBEDSOCIALHASHTAG.getCurrentUrl())) {
            var encodedUrl = window.encodeURIComponent(EMBEDSOCIALHASHTAG.getCurrentUrl());
            if (queryParamString) {
                queryParamString = queryParamString + "&origin=" + encodedUrl
            } else {
                queryParamString = "?origin=" + encodedUrl
            }
        }
        if (iframes.length <= 0) {
            var ifrm = document.createElement("iframe");
            var iframeId = "embedIFrame_" + albumRef + Math.random().toString(36).substring(7);
            var srcIfrm = "https://embedsocial.com/api/pro_hashtag/" + albumRef + "/" + queryParamString;
...
if ("IntersectionObserver"in window) {
    function callVisible(e, t) {
        for (j in e)
            e[j].isIntersecting && EMBEDSOCIALHASHTAG.getEmbedData(e[j].target.getAttribute("data-ref").trim(), e[j].target)
    }
}
function handleMutations(m) {
    for (var i = 0; i < m.length; i++) {
        for (var j = 0; j < m[i].addedNodes.length; j++) {
            var node = m[i].addedNodes[j];
            if (node.nodeType === 1 && node.classList.contains("embedsocial-hashtag") && node.getAttribute("data-dynamicload") === "yes") {
                EMBEDSOCIALHASHTAG.getEmbedData(node.getAttribute("data-ref"), node)
            }
        }
    }
}
var dw = document.querySelectorAll('.embedsocial-hashtag[data-dynamicload="yes"]');
if (dw.length > 0 && "MutationObserver"in window) {
...
function standardLoad(e) {
    for (var j = 0; j < e.length; j++) {
        ...
        if ("yes" === t.getAttribute("data-lazyload") && "IntersectionObserver"in window)
            new IntersectionObserver(callVisible,{}).observe(t);
        else
            EMBEDSOCIALHASHTAG.getEmbedData(o, t)
    }
}
